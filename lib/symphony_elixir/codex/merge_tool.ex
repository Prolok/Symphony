defmodule SymphonyElixir.Codex.MergeTool do
  ...
  alias SymphonyElixir.{CommentCheckpoint, Config, PathSafety, RuntimePaths}
  ...
  def invoke(arguments, opts \\ []) do
    with %{"head_sha" => head} when is_binary(head) <- arguments,
         ...
         {:ok, workspace} <- workspace(issue),
         python when is_binary(python) <- System.find_executable("python3") do
      run(python, workspace, issue, head, opts)
    else
      {:error, _} = error -> error
  ...
  defp run(python, workspace, issue, head, opts) do
    script = Path.join(RuntimePaths.workflow_dir(), ".codex/skills/symphony-land/land_watch.py")

    env =
      Config.without_linear_secret(%{"SYMPHONY_ISSUE_IDENTIFIER" => issue.identifier})
