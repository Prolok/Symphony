defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_linear_scope_line({:ok, {:project, project_slug}}) do
    colorize("│ Project: ", @ansi_bold) <> colorize(linear_project_url(project_slug), @ansi_cyan)
  end

  defp format_linear_scope_line({:ok, {:team, team_key}}) do
    team_scope = "#{team_key} — #{linear_team_url(team_key)}"
    colorize("│ Team: ", @ansi_bold) <> colorize(team_scope, @ansi_cyan)
  end
  ...
  def format_running_summary_for_test(running_entry, terminal_columns \\ nil) do
    ...
    format_running_summary(running_entry, running_event_width(terminal_columns))
  end
  ...
  defp format_retry_summary(retry_entry) do
    ...
    "│  #{colorize("↻", @ansi_orange)} " <>
      ...
      format_retry_due(due_in_ms) <>
      error
  end

  defp format_retry_due(nil), do: colorize(" paused", @ansi_cyan)

  defp format_retry_due(due_in_ms) do
    colorize(" in ", @ansi_dim) <> colorize(next_in_words(due_in_ms), @ansi_cyan)
  end
