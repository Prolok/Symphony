defmodule SymphonyElixir.Linear.CommentMutations do
  ...
  @spec state_updates(map(), boolean()) :: {:ok, [map()]} | {:error, term()}
  def state_updates(%{"query" => query} = payload, all? \\ false) do
    with {:ok, %{input: document}} <- Parse.run(%L.Source{body: query}),
      ...
      if operation.operation == :mutation do
        ...
        {:ok, state_fields(expand(operation.selection_set.selections, fragments, []), variables, all?)}
      else
        {:ok, []}
      end
    else
  ...
  @doc "Extract description writes for the managed fixture's source-bound ownership journal."
  @spec description_updates(map()) :: {:ok, [map()]} | {:error, term()}
  def description_updates(%{"query" => query} = payload) do
    with {:ok, %{input: document}} <- Parse.run(%L.Source{body: query}),
         {:ok, operation} <- select_operation(document, payload["operationName"]) do
      if operation.operation == :mutation do
        variables = variable_values(operation, payload["variables"] || %{})
        fragments = Map.new(Enum.filter(document.definitions, &match?(%L.Fragment{}, &1)), &{&1.name, &1})
        {:ok, description_fields(expand(operation.selection_set.selections, fragments, []), variables)}
      else
        {:ok, []}
      end
    else
    ...
    _ -> {:error, :invalid_description_mutation}
  catch
    :invalid_comment_mutation -> {:error, :invalid_description_mutation}
  end

  defp description_fields(selections, variables) do
    selections
    |> Enum.filter(&enabled?(&1.directives, variables))
    |> Enum.flat_map(fn
      %L.InlineFragment{selection_set: set} ->
        description_fields(set.selections, variables)

      %L.Field{name: "issueUpdate", arguments: arguments} ->
        values = Map.new(arguments, &{&1.name, value(&1.value, variables)})

        if is_map(values["input"]) and Map.has_key?(values["input"], "description"),
          do: [%{"id" => values["id"], "description" => values["input"]["description"]}],
          else: []
